...
DEFAULT_DOI = "10.5281/zenodo.20038194"
...
def resolve_model(doi: str, models_dir: Path, no_cache: bool) -> Path:
    folder_name = _doi_to_folder_name(doi)
    model_folder = models_dir / folder_name
    pt_files = _find_pt_files(model_folder)
    if no_cache or not pt_files:
        print(f"Downloading {doi} to {model_folder}")
    ...
    if len(pt_files) > 1:
        ...
        raise RuntimeError(f"Multiple .pt files in {model_folder}: {names}")
    return pt_files[0]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "downloading the model from Zenodo by DOI. "
            "Use it with the sdcpsam conda env, for example: "
            r"C:\Users\p000881\AppData\Local\miniconda3\envs\sdcpsam\python.exe test_fromdoi.py"
        )
    )
    parser.add_argument("--doi", default=DEFAULT_DOI, help="Zenodo DOI, e.g. 10.5281/zenodo.20038194")
    parser.add_argument(
        "--models-dir",
        type=Path,
        default=None,
    ...
    parser.add_argument("--no-cache", action="store_true", help="Always re-download even if already cached.")
...
def main() -> int:
    ...
    pt_path = resolve_model(args.doi, models_dir, args.no_cache)
    ...
    for device_name in args.devices:
        ...
        print()
        print(f"Loading model on {device} from {pt_path}")
        load_start = time.perf_counter()
        model = StarDist2D.from_checkpoint(pt_path, device=device)
        synchronize(model.device)
